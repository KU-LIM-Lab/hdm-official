...
class AFNO2D(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.sparsity_threshold = config.model.sparsity_threshold
        self.num_blocks = config.model.num_blocks
        self.block_size = self.hidden_size // self.num_blocks
        self.hard_thresholding_fraction = config.model.hard_thresholding_fraction
        self.hidden_size_factor =config.model.hidden_size_factor
        self.scale = 0.02

        self.w1 = nn.Parameter(self.scale * torch.randn(2, self.num_blocks, self.block_size, self.block_size * self.hidden_size_factor)).to('cuda')
        self.b1 = nn.Parameter(self.scale * torch.randn(2, self.num_blocks, self.block_size * self.hidden_size_factor)).to('cuda')
        self.w2 = nn.Parameter(self.scale * torch.randn(2, self.num_blocks, self.block_size * self.hidden_size_factor, self.block_size)).to('cuda')
        self.b2 = nn.Parameter(self.scale * torch.randn(2, self.num_blocks, self.block_size)).to('cuda')
